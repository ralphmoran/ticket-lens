...
**Multi-profile resolution (backlog #12, fixed 2026-08-11):** Lever 1 (`resolveConnection(ticketKey, ...)`) resolves by ticket-key prefix when a ticket key is available. Lever 2 (`resolveProfile(..., { cwd })`, in the Stop hook) originally always passed `null` for the ticket key, so it fell through to cwd/`projectPaths` then the default profile regardless of which ticket the session was actually about. Fixed by having `scanTranscript()` (`recall-nudge-lib.mjs`) return the literal text of the first matched ticket key (a new `ticketKey` field, alongside the pre-existing `sawTicketKey` boolean) and threading it into the Stop hook's `resolveProfile(ticketKey, { cwd })` call — both levers now resolve the same profile whenever a ticket key is available. Covered by dedicated real-subprocess tests in `recall-nudge-stop.test.mjs` proving both directions (a non-default profile's stricter setting applying, and its looser setting applying) that only pass with the fix present.
