...
- ~~**Content-based auto-tagging**~~ **Shipped (2026-07-31).** See "Recall content-linked auto-tagging + human-readable timestamps — shipped" section below.
- ~~**Human-readable created timestamp on Recall notes**~~ **Shipped (2026-07-31).** Same section below.

---

## Recall content-linked auto-tagging + human-readable timestamps — shipped (2026-07-31)

10-iteration proposal/refutation loop run before any code, per the user's explicit request — full loop in solopreneur memory (`project_recall_tags_timestamps_2026_07_31.md`, if present) or reconstructable from the session that shipped this.

**Tagging — real finding, changed the planned approach:** traced `--tags`/MCP `tags` end-to-end through `note-command.mjs`, `recall-vault.mjs`, `mcp-server.mjs` — confirmed zero code-side tag generation exists anywhere; tags are 100% whatever the caller supplies. Given `SKILL.md`'s entire Recall-capture flow is built around *autonomous* AI capture (Claude decides when to save, drafts the body, calls `note add` itself) and gave zero guidance on tag quality, the observed bad tags ("jtb", "ticketlens", "gotcha") are almost certainly AI-authored via this exact skill, not human-typed. **Decision: prompt-level fix only, no extraction code** — added concrete tag-quality guidance to `SKILL.md`'s "How to dispatch the call" section and to the `recall_add` MCP tool's `tags` input-schema description (an MCP client never reads SKILL.md, so both surfaces needed the same guidance). Code review confirmed this reasoning holds: no code path could have generated the bad tags on its own.

**Timestamps — real finding, narrower scope than the original backlog note assumed:** `styled-assembler.mjs`'s bare-date pattern (`d.created.split('T')[0]`) is used consistently for ticket `Created:`/`Updated:`, comment dates, and linked-ticket comment dates — a shared convention across unrelated features, not something specific to Recall. Applying `timeAgo()` broadly would have touched already-shipped ticket-brief formatting nobody asked to change. **Scoped to exactly two Recall-note display sites**: `styleRecallResults` (the actual `ticketlens recall <query>` output) and `recall-vault.mjs`'s `rebuildIndex` (the Obsidian-facing index.md). `timeAgo()` itself reused unchanged in logic — gained only an optional injectable `{ now }` parameter for testability (it had zero test coverage anywhere despite being used in production triage output since it was written). A code-review-driven regression-lock test now proves `styleBrief`'s ticket dates stay bare, unaffected.

**Console scope, investigated and explicitly excluded:** `Console/Admin/Recall.vue` renders whatever tags/timestamp data the CLI already sends — `note.tags` is looped over generically (line 171), so better tags from the CLI side show up automatically with zero Console code changes. `Recall.vue`'s own `formatDate` (line 34) has the identical bare-date issue, but is a separate Vue codebase with its own formatting convention and would require the full UI-verification pipeline (Chrome DevTools MCP, dev server, screenshots) — logged as a smaller, separate future item rather than bundled into this CLI-only ship.

**Code review: 0 CRITICAL, 0 HIGH.** 1 MEDIUM (fixed): three test files compute their expected `timeAgo()` output via the same function rather than a hardcoded literal, which verifies field-wiring correctness but not `timeAgo()`'s own arithmetic independently — added clarifying comments noting `config.test.mjs`'s deterministic-clock test suite is what actually pins the arithmetic. 1 LOW (fixed): `timeAgo()`'s JSDoc updated for the new `{ now }` parameter.

**Residual, accepted gaps, not solved here:** a user typing `ticketlens note add --tags=...` directly in a bare shell gets no benefit from the SKILL.md fix (no AI in that loop) — the skill's whole design center is AI-driven capture, so this is a known, secondary limitation. Console's own bare-date display remains a separate, smaller future item.

**Process note:** full regression suite went 2696 → 2709 tests across this feature (13 net new), zero regressions at every checkpoint.

**Not published to npm** — separate manual step, always `--tag=beta` per standing instruction, same as every other feature in this initiative.
