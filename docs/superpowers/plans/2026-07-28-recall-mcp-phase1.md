...
- **Content-based auto-tagging (backlog, requested 2026-07-30, refined 2026-07-30).** Today `tags` on `note add`/`recall_add` are 100% manual (`--tags=a,b` / MCP `tags` array — confirmed by reading `note-command.mjs`, no auto-generation exists anywhere). User feedback: real tags observed in the Console are generic project/category labels ("jtb", "ticketlens", "gotcha", "php53", "ticket-management") that don't reflect what a given note's body actually contains — when a note is queried/searched to help a *different* developer than the author, generic tags provide little discovery signal. **Explicit requirement (2026-07-30 follow-up):** generated tags must be directly linked/related to the recall's own body content — not generic project/category labels — so search relevance actually improves. **Possible lower-effort implementation path (2026-07-30 follow-up):** the user suggested this could live in `skills/jtb/SKILL.md` and/or the MCP tool description/"rules" (the `recall_add` tool's `description` field in `mcp-server.mjs`) rather than new CLI code — i.e., instruct the calling AI (Claude Code or any MCP-compatible harness, which is very often the one actually invoking `recall_add`/`note add` on the user's behalf) to derive tags from the note body's real content at call time, instead of teaching it nothing and letting it default to generic project-name tags. Confirmed by reading both: `SKILL.md` line 234 shows only a bare `note add --tags=a,b` syntax example with no guidance on tag *quality*; `mcp-server.mjs`'s `recall_add` tool description (`TOOLS[0]`) likewise says nothing about how `tags` should be chosen. This is a real alternative (or complement) to a code-level extraction feature — much lower effort (prompt/doc-only change, no new adapter/module), but only helps when an AI is the caller, not a human typing `ticketlens note add` directly at a terminal; a full solution likely wants both prongs eventually. Still needs its own scoping pass regardless of path chosen: should content-tags supplement or ever override manual `--tags`? Does the Console's tag search/filter UI need changes to surface a "content-derived" vs. "manual" tag distinction?
- **Human-readable created timestamp on Recall notes (backlog, requested 2026-07-30).** Confirmed by reading `recall-vault.mjs`: `writeNote` already stores `created: now().toISOString()` (line 77), but the only place it's ever rendered (the vault index, line 351) truncates it to `n.created.split('T')[0]` — a bare date, no time-of-day, no relative phrasing. User wants a genuinely human-readable timestamp shown when a recall is created/displayed (e.g. "3:45 PM" or a relative form like "2 days ago"). A reusable helper already exists and does exactly this: `timeAgo(dateStr)` in `config.mjs` ("Human-readable relative time from an ISO date string" — `${mins}m ago`/`${hours}h ago`/`${days}d ago`), already used elsewhere in the CLI (e.g. `stats`/`triage` output) — this is likely a small, low-risk change: apply `timeAgo()` (or a fuller human format) at the display layer (vault index render, `recall` search output, Console note list) rather than changing the stored ISO format itself.
