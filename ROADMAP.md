...
| C-T1 | ~~Feature~~ | ~~**Handoff brief (`--handoff`)**~~ | Done (v0.1.21). `ticketlens TICKET-KEY --handoff` generates an AI-powered handoff brief from the ticket's comment thread: what was attempted, current blockers, open questions, and a recommendation. BYOK (Anthropic/OpenAI/Groq) or `--cloud`. Pro-gated. v0.1.24: added Groq free-tier BYOK (`llama-3.1-8b-instant`), AI provider selection (`ticketlens config set aiProvider groq`), per-command override (`--provider=groq\|openai\|anthropic`). 1008 tests. | ~~Small–Medium~~ |
