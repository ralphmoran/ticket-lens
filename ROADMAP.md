...
| 49f | Feature | **`ticketlens doctor` progress UX** | **Done 2026-08-06 (pending commit/publish).** User feedback on the shipped 49c: `doctor` gave no visible progress while running and the multi-profile connectivity failure summary dropped its classified hint. Fixed: `doctor-command.mjs` now runs checks sequentially with a "○ Checking &lt;label&gt;…" line per check, erased once resolved (`\x1b[A\r\x1b[2K`, same pattern as `login-flow.mjs`), gated on `format === 'plain' && stream.isTTY` — no effect on `--format=json`, piped output, or the MCP path. `connection-tester.mjs`/`doctor-checks.mjs` now thread `classifyError`'s `hint` through the full-sweep connectivity summary, so a multi-profile failure carries the same "why + next step" every other check already had. Live-verified via `--format=json` and a real pty (`script(1)`). 2973/2973 tests pass, code-reviewer: 0 CRITICAL/HIGH. | Small–Medium |
