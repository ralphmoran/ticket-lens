...
| 19c | Feature | **Response time metrics — Console** | **NEXT.** Console `/console/admin/stats` page with charts: avg response-time trend (line, 30d), clear-rate trend (line), urgency breakdown (bar/donut, latest snapshot), team comparison table (one row per team member, sortable). Data fed from existing `triage-history` snapshots already written by `ticketlens triage`. Manager view: aggregated across all team members for the sprint. | Medium |
