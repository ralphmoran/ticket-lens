...
| C-T3 | ~~Feature~~ | ~~**Standup/PR generator**~~ | Done (v0.1.19). `ticketlens standup` scans `git log --since` (default 24h), groups commits by ticket key, optionally fetches ticket summaries via a Jira profile, and outputs a standup brief or PR body. `--since=N` (hours) or git date string. `--format=standup\|pr`. `--plain` for piping. Typo detection for all flags. 1036 tests. | ~~Medium~~ |
