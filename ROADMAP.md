...
| 49b | Feature | **Full MCP tool coverage — every CLI action available to an AI harness** | **Backlog, unscoped (added 2026-08-01).** User request: "all TL actions should be available in the MCP." Current MCP surface is 9 tools (`recall_add`, `recall_search`, `ticket_comment`, `ticket_transition`, `ticket_assign`, `ticket_duplicates`, `ticket_link`, `ticket_update`, `ticket_create`) against 35 CLI subcommands — a real, sizeable gap. Read/analysis commands not yet exposed: `triage`, `compliance`, `standup`, `review`, `collisions`, `stats`, `history`, `sync`. Needs a scoping pass before a plan: (1) which of the remaining commands are genuinely "actions an AI agent should be able to take" vs. local-session/account-admin commands that should stay CLI-only regardless (`login`/`logout`/`config`/`license`/`activate`/`profiles`/`switch`/`cloud-keys`/`init`/`install-hooks`/`update-skill`/`version`/`schedule`/`ledger`/`delete`) — a literal 1:1 port is very likely wrong; (2) tier gating for each newly-exposed tool, consistent with the existing Pro+ pattern; (3) whether read-only tools (`triage`, `stats`, `history`) need the same audit-logging/cooldown treatment the write tools have, or a lighter one. | Large |
