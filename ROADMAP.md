...
| 19d | Feature | **Stale status detection** | **Backlog — needs product review.** Current urgency model is comment-driven; tickets stuck in the same non-terminal status for N days with no activity (no comments, no transitions, no commits) are silently `clear`. Proposed: new `stale` flag in `attention-scorer.mjs` triggered when `ticket.updated` has not changed for ≥N days (configurable, e.g. 14d default). Escalation pipeline: `in-progress → stale → aging → needs-response` style workflow for status-duration-driven urgency. Would feed: Slack alert in `EvaluateAlertsJob`, new `stale` band in Stats page trend chart, Alert settings toggle + threshold input. Discovered via CoreNexus tickets at 90d "In Progress" with no flags fired. Design not final. | Medium |
