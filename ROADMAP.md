...
| 49d | Feature | **Configurable Recall capture strictness** | **Backlog, unscoped (added 2026-08-04).** User request: a mechanism to set how strict or lenient the jtb skill's Recall-capture nudge (`recall-nudge-stop.mjs` + the "did anything non-obvious come up?" guidance in `SKILL.md`) should be about deciding something is worth saving as a note. Explicitly **not** a filter on existing notes — a dial on the judgment call made at capture time, before a note is ever written. User offered candidate framing words — criteria, importance, severity, quality — as illustrative starting points only, not a decided taxonomy; the right term(s) and whether this is a single scalar (e.g. a "strictness" level) or multiple independent dimensions still needs review before scoping. Open questions, none resolved yet: where the setting lives (per-profile in `~/.ticketlens/profiles.json`? a new global config key? a Console-managed team default, same pattern as Recall queue settings in L-4/`recall-settings-sync.mjs`?); whether it's enforced by the AI's own judgment via stronger/weaker SKILL.md prompt language (same mechanism as the L-12 tag-quality fix) or by an actual code-side gate; and how it interacts with `note add`'s existing structural/secret-scan checks, which are correctness gates, not relevance gates, and should stay separate from whatever this becomes. | Medium |
