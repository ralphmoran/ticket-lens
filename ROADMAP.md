...
| 49f | Feature | **`ticketlens doctor` progress UX** | **Backlog, unscoped (added 2026-08-06).** User feedback on the shipped 49c: `doctor` currently gives no visible progress while running — no indicator of which check is currently executing, so it reads as "stuck" for the run's duration — and the final report lacks detail/actionable suggestions once it does return. Scope: a loader/progress indicator per check (matching the check list already defined in the 49c design spec), plus richer per-check output (what failed, why, and a concrete next step), not just pass/fail. | Small–Medium |
