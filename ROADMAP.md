...
| 19d | Feature | **Stale status detection** | **Session A (CLI) done (v0.9.0). Session B (Backend) done (fcd4bad). Session C (Console polish) done (4d59dcd). Live test Steps 3–5 pending (GET /v1/statuses, POST /v1/profile-sync stale-rule merge, Stats page stale band).** Design complete. New `stale` urgency level between `aging` and `clear` (`needs-response > aging > stale > clear`). Triggered when a ticket has been in the same specific Jira status for ≥N days (configurable, separate `stale_days` threshold). Data source: Jira changelog (`?expand=changelog`) — parses last status transition date, zero extra API calls. Reason string: "Stuck in 'In Review' for 23d". **Three-layer feature:** (1) CLI — `jira-client.mjs` changelog parsing, `attention-scorer.mjs` new stale rule application, sync pulls rules from backend, stale display in triage output. (2) Backend — `workflow_rules` table (type='stale', config jsonb, team-level + user-level override, user rule takes precedence), `GET /v1/sync` enriched with applicable rule, `EvaluateAlertsJob` new stale alert type, Stats page new stale band, status cache endpoint derived from `triage_snapshots`. (3) Console — new `/console/admin/rules` "Workflow Rules" page (first rule type: Stale Detection), status multi-picker with local cache + refresh button, team rule managed by manager, user-level override in user settings, Pro gate. Jira statuses cached locally per profile (`knownStatuses` in profile config), refreshed via Console "Refresh" button or `ticketlens sync`. Sync: auto on push + explicit `ticketlens sync`. Action: alert / escalate / both. Pro-tier gate. Establishes Console as lifecycle rules engine — future rule types (SLA, priority escalation) follow same pattern. | Large |
| 20 | Feature | **Live Console Events (SSE)** | **Backlog.** Real-time push from Console to connected browser users via Server-Sent Events (SSE). Laravel `response()->stream()`, browser native `EventSource` — no external dependency (Pusher/Reverb deferred). Use cases: (1) Admin saves a workflow rule → team members' Console updates without page refresh. (2) Live triage feed — new stale/aging tickets surface in real-time. (3) Real-time alert delivery in Console UI alongside Slack. CLI users covered by existing sync-on-push model. Scope: SSE endpoint per team channel (auth via Sanctum token in EventSource URL param or cookie), `RuleChangedEvent` + `TicketStatusChangedEvent` broadcasts, Console composable `useServerEvents()` that subscribes on mount and dispatches to relevant Pinia store slices. No polling fallback needed — EventSource reconnects automatically on drop. | Medium |
