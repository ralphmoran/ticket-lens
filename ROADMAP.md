...
| 12f | ~~Feature~~ | ~~**Recall push resilience — offline retry queue**~~ | Done. A team push failing for a transient reason (network error, timeout, 5xx) no longer gets a one-shot "saved locally only" warning and nothing else — the note is queued in `recall-pending.json` (`recall-queue.mjs`) and retried automatically, at most once every 15 minutes, on every command (not just `note add`/`recall`) via a short-timeout background check in `bin/ticketlens.mjs`. `ticketlens recall sync` still flushes on demand. Session-expired (401) and not-entitled (403) failures are deliberately never queued — retrying those can't succeed without the user acting. Each queued entry is tagged with a hash of the account token that queued it, so a later account switch never flushes a note under the wrong login; the queue is capped at 200 entries and expires entries after 30 days (measured from first-queued, not last-retried, so a perpetually-failing entry can't stay alive forever by retrying its own clock). Zero new deps. | ~~Medium~~ |
| 12g | ~~Feature~~ | ~~**`note delete` + every-command auto-flush**~~ | Done. `ticketlens note delete --id="..." [--ticket=KEY]` removes a note from the local vault (`deleteNoteFn` in `recall-vault.mjs`, already used internally for pull-tombstone sync). Local-only: a note already pushed to a team stays with teammates until a manager deletes it in Console. `maybeAutoFlush()` in `recall-queue.mjs` now runs from every command's entry point, not just `note add`/`recall`, with a 4s per-request timeout so a down backend never stalls an unrelated command; the existing 15-minute global cooldown is unchanged. Zero new deps. | ~~Small~~ |
| 12h | ~~Feature~~ | ~~**Manager-controlled retry-queue settings**~~ | Done. Retry cooldown, per-request timeout, max queue size, and queued-note expiry are now set by a team manager at `console/admin/recall` (`recall_settings` table, IDOR-guarded, validated bounds), applying to every member's CLI; solo users get the same platform defaults through the same endpoint (`GET /v1/recall/settings`). CLI fetches live at the moment a setting is actually needed (queue non-empty, a push just failed, or the new read-only `ticketlens recall settings` command) — not on a background timer — so a manager's Console change is visible on the very next relevant command, with a fixed 3s fetch timeout and silent fallback to a local last-known-good cache (account-scoped, never served across a login switch) or platform defaults if offline. `enqueueNote()` in `recall-queue.mjs` became async as part of this; the vast majority of commands (empty queue) still never touch the network. Zero new CLI deps. | ~~Medium~~ |
...
| 24b | Feature | **Multi-agent AI consensus compliance (`--consensus`)** | **Backlog, unscoped (added 2026-08-01).** `ticketlens compliance TICKET --consensus` — instead of TicketLens's own single deterministic requirement/diff matcher, route the same requirements-vs-code-diff review through 3-5 different AI agents/models (BYOK across providers — user supplies API keys/tokens for each), each producing an independent review, then synthesize into one consolidated report via a self-improvement/refinement loop across the multiple viewpoints. Open design questions, none resolved yet: which providers count as "agents" (different vendors vs. different prompts on one vendor?), how disagreement between agents is surfaced/reconciled (voting? one arbiter pass? show all N verdicts?), cost/API-key management (BYOK per provider, stored where, how many calls per run), whether this runs purely local-CLI, via an n8n workflow, on the `ticketlens-api` backend, or some split of the two, and tier gating (likely Team+, given the existing compliance feature is already the Free→Pro lever). Needs its own scoped research pass before a plan — same treatment given to `ticket_create`/`ticket_comment` attachments. | Large |
...
| C0-10 | ~~Feature~~ | ~~**Triage Rules — Console-managed custom attention rules**~~ | Done. Extends `/console/admin/rules` (`WorkflowRule` gains `type='custom'` alongside `stale`, enum widened via driver-conditional migration) with a second card: managers configure force-urgent/ignore rules (`match: {priority,label,status,keyPrefix}`, first-match-wins) that were previously only settable by hand-editing local `~/.ticketlens/profiles.json` (Feature 10). `ticketlens sync` (already the pull mechanism for the team stale rule) now also pulls `attention_rules` — no new CLI sync plumbing needed, one field added to `serverToCliProfile()`. The CLI-side matcher (`attention-scorer.mjs::applyCustomRule`) is unchanged, already tested. Bundled fix: `ProfileSyncController::resolveTeamStaleRule()` resolved a manager's group via membership-only (`groups()->first()`) instead of ownership-first (`ownedGroup ?? groups()->first()`, matching `RulesController`) — a manager who owns a group but isn't a member of it synced the wrong (or no) team rule; fixed for both stale and custom, regression-proven. `schedule`/`notify` rule actions remain CLI-side no-ops, explicitly backlogged pending their own spec. Pro+ gated, same as stale. Zero new CLI deps. |
| C0-11 | ~~Feature~~ | ~~**Triage Rules — notify/schedule actions + priority/label sourcing**~~ | Done. Closes the C0-10 backlog item. `notify`/`schedule` join `force-urgent`/`ignore` as valid custom-rule actions (`RulesController::saveCustom`), gated server-side by an existing `SlackIntegration` for the group (rejected at save time, not just hidden in the UI). Server is the sole source of truth for notify/schedule matching — a new `CustomRuleMatcher` service (PHP port of `attention-scorer.mjs`'s predicate, parity-pinned by a shared fixture) independently re-evaluates every pushed ticket; the CLI's local force-urgent/ignore path is untouched. `notify` posts to the team's Slack channel via a new `EvaluateCustomNotifyRulesJob` (deliberately separate from `EvaluateAlertsJob`, which is coupled to an unrelated model and cooldown shape); `schedule` lists matches in the next weekly digest (`SendSlackDigestJob`), recomputed fresh each send — no new table. Priority/label real-value sourcing in the rule builder: `ticket-payload.mjs` now includes `priority`/`labels` in the push payload (previously omitted even though `jira-client.mjs` already fetched both), validated and bounded server-side (`PushRequest`), aggregated the same way `known_statuses` already is (`TriageSnapshot` fallback — the per-profile cache column path was confirmed to have no write path anywhere in the codebase, so it was not extended). Bundled fix: pre-existing unescaped Slack mrkdwn injection (`<!channel>` mass-ping, `<url\|text>` link spoofing) in `EvaluateAlertsJob`/`SendSlackDigestJob`'s message building, closed at its origin call sites via a new `SlackMrkdwnEscaper`, not just the new notify path. Pro+ gated for custom rules generally (unchanged); notify/schedule additionally require Team-tier Slack. Zero new CLI deps. |
| C0-12 | ~~Feature~~ | ~~**Rules page — team connection visibility**~~ | Done. `/console/admin/rules` now shows which team members haven't connected a tracker profile yet, right where Force urgent/Ignore rules are configured — mirrors the existing Slack-connection banner for Notify/Add to digest. `RulesController::buildRuleData()` diffs the group roster against `TrackerProfile` rows (zero new queries — reuses the existing member/profile lookups), returns `unconnected_members`; `Rules.vue` renders the banner and links to Connections. Also added "Connections" to the main Console sidebar nav (`ConsoleLayout.vue`) — previously only reachable via Settings. Zero new deps, zero new queries, no migration. |
| C0-13 | ~~Fix~~ | ~~**`--push` dropped actively-worked tickets from Team notify/schedule rules**~~ | Done. `--push` (`fetch-my-tickets.mjs`) sent only the `actionable` set (needs-response/aging/stale), silently excluding `clear`-urgency tickets — a High/Highest-priority ticket the assignee was actively, correctly working on (recently updated, no unanswered comment) never reached the server, so a manager's C0-11 priority-based notify/schedule rule could never match it regardless of priority. Fixed: `--push` now sends every scored ticket except ones excluded by a local `ignore` custom rule (`pushable = sortByUrgency(scored.filter(urgency !== 'ignore'))`); terminal display, `--digest`, `--export`, `--share` are unchanged (still actionable-only). Bundled fix: `scoreAttention()` never carried `priority` in its return shape, so the CLI's own triage tables showed staleness but never priority — added to all 7 return branches and threaded into the plain, styled, and interactive terminal renderers (new `priorityColor()` helper in `styled-assembler.mjs`, also applied to the existing single-ticket brief view for consistency). Backend hardening: `PushRequest.php` gained `max:300` on the `tickets` array and `max:20` on each ticket's `flags` array — defense-in-depth now that `--push` legitimately carries a larger set than before. Zero new CLI deps. |
...
| C-T10 | ~~Feature~~ | ~~**Triage config management — personal sort order + manager notify cooldown**~~ | Done. Two independent, previously-scoped-as-one requests split during brainstorming: (1) **Personal sort order** — self-service per-user preference, not a team policy. CLI: `sortBy: 'urgency'\|'priority'` field in a profile's `~/.ticketlens/profiles.json` entry (same hand-edited-field precedent as `attentionRules`/`staleRule`), plus a one-run `--sort=priority\|urgency` override flag (same override relationship `--stale=N` has to `staleDays`). `sortByUrgency()` ranks by priority first (Highest→Lowest, unknown last) then urgency as tiebreaker when `sortBy==='priority'`; unset is byte-identical to prior behavior. Console: new `users.triage_sort_preference` column (default `urgency`), exposed via the existing self-service `AccountController`/`Account.vue` (not manager-gated), independently governs `Console/Queue.vue`'s client-side sort — CLI and Console preferences do not sync by design. (2) **Team-wide notify cooldown** — `cooldown_hours` (1–720, default 4) added to the `custom`-type `WorkflowRule.config`, manager-configurable via `/console/admin/rules`, replacing the hardcoded 4h in `EvaluateCustomNotifyRulesJob`. (3) **Discoverability** — cross-link banners between `/console/admin/rules` and `/console/admin/jira` (explicitly kept as separate pages, no merge, per direct instruction), plus a missing `ConsoleLayout.vue` sidebar entry for the Jira Connection page. Two pre-existing gaps this surfaced (stale-days threshold, status filter) turned out to already be manager-configurable via the existing `stale`-type `WorkflowRule` and `TeamJiraConfig` — not net-new. | Medium |
...
| 49b | Feature | **Full MCP tool coverage — every CLI action available to an AI harness** | **Scoped 2026-08-11** (added as backlog 2026-08-01; user request: "all TL actions should be available in the MCP"). Current MCP surface is 10 tools, not 9 as originally logged (`doctor` shipped 2026-08-05/08 after this entry was written) — `doctor`, `recall_add`, `recall_search`, `ticket_comment`, `ticket_transition`, `ticket_assign`, `ticket_duplicates`, `ticket_link`, `ticket_update`, `ticket_create` — against 27 unwrapped CLI surfaces. Architecture confirmed reusable: every MCP tool is a pure transport adapter wrapping an existing CLI-layer function, inheriting its tier gate for free — zero new logic per tool. Scoping pass confirmed a literal 1:1 port is wrong, as this entry originally suspected (full table: project memory). **11 net-new tool candidates approved**, port order `fetch`/`triage` first → `ledger` last: `fetch` (biggest single gap — the core ticket-brief read, currently invisible to any non-Bash MCP harness; missing from this entry's own original draft), `triage`, `compliance`, `review`, `standup`, `pr` (also missing from the original draft), `stats`, `history`, `collisions`, `ledger` — plus a hidden gap fixed as new `recall_update`/`recall_delete` tools (`recall_add` already lets an agent create a Recall note but gave it no way to fix or remove one; CLI equivalents are `note patch`/`note delete`). Gated flags on `triage`/`stats` are exposed now, not deferred. Read-only tools get the tier-check funnel only, not the `ticket_*` write tools' confirm/cooldown/audit-log machinery — they never mutate the tracker. Reclassified to CLI-only/skip: `sync` (mutates local credential state, not an analysis output — this entry originally miscategorized it as read/analysis), `recall sync` (redundant, `recall_search` already force-pulls fresh every call), `recall settings`, plus the original admin set (`login`/`logout`/`config`/`license`/`activate`/`profiles`/`switch`/`cloud-keys`/`init`/`install-hooks`/`update-skill`/`version`/`schedule`/`delete`) and `cache` (omitted from the original draft). **`fetch` shipped 2026-08-12** (`ticket-lens@b93f199`) and **`triage` shipped 2026-08-12** — 2 of the 11, local/not pushed. `triage`'s flag-tier split from the scoping decision now includes 4 fields missed in the original scoping pass: `project`/`label`/`priority` (Team — same JQL-filter family as `assignee`/`sprint`) and `sort` (Free — was never gated in the CLI at all). `--push`/`--share` deliberately excluded (Console-notification/human-sharing side effects, not "what needs attention" read value). Real architectural fix required: `fetch-my-tickets.mjs`'s `run()` hardcoded `process.stderr` at ~15 sites even though the helper functions it calls (`createSession`/`promptProfileSelect`/`showUpgradePrompt`) already accepted an injectable `{stream}` param — threaded a new `opts.stream` (default `process.stderr`, zero CLI behavior change) through all of them so the MCP server can capture errors instead of losing them to a stderr it never reads. `--digest` intentionally prints nothing to stdout on success (pre-existing locked test) — the MCP wrapper special-cases this so a successful silent delivery isn't misreported as a failure. **`compliance` shipped 2026-08-12** — 3 of the 11, local/not pushed. Wraps the same `fetch-ticket.mjs` `run()` the `fetch` tool already wraps (`compliance` is just another subcommand), reusing `runFetchTicketFn` — no new dependency. Two leaks fixed: the block's ~6 direct `process.stderr.write` sites (same treatment as `fetch`/`triage`), plus a second, distinct leak — `runComplianceCheck` was never given a `stream` option at all, so its Pro-tier usage-gate's `showUpgradePrompt` always wrote to real stderr regardless of any injected `printErr`. `callFetch`/`callCompliance` were byte-identical apart from the args-builder and fallback error string — extracted a shared `callFetchTicketRun`. Also fixed a real SKILL.md gap found along the way: the standalone `ticketlens compliance PROJ-123` command (what `install-hooks` wires into the pre-push git gate) was previously undocumented entirely — SKILL.md only covered the separate `--compliance` fetch-time flag. **`standup`/`pr`/`review` shipped 2026-08-12** — 6 of the 11, local/not pushed. All three wrap the same `fetch-ticket.mjs` `run()` `fetch`/`compliance` already wrap, reusing `callFetchTicketRun` — no new dependency. Same stderr-injection treatment as prior tools: `pr` (5 direct-write sites), `review` (10 sites plus a `sErr`/`makeSpinner` pair previously hardcoded to real `process.stderr.isTTY`), `standup` (10 sites, same hardcoding) — `makeSpinner` itself made injectable via a `{isTTY, write}` param. Code review caught a second, distinct leak in `pr`: `assemblePr`'s internal `runComplianceCheckFn` call passed no `stream`, so its Pro-gate upgrade prompt (drawing from the same Free-tier 3-checks/month counter as `compliance`) still wrote to real stderr even after the dispatch block's own writes were fixed — closed via a wrapping closure in the dispatch block rather than changing `pr-assembler.mjs`'s public signature. `review`'s own Pro-gate is a plain license check, confirmed NOT drawing from that counter — README/`printMcpHelp()` wording tightened after review flagged it as ambiguous. SKILL.md previously had zero mentions of any of these three commands; added full CLI-usage + MCP-callout sections for all three plus a `jtb-skill-version` bump (0.36.0→0.37.0). 3352 tests (recursive `npm test` glob — a hand-typed non-recursive glob silently undercounts by ~165 tests, hit mid-session), 0 failures, code-reviewer 0 CRITICAL / 1 HIGH (fixed) / 2 LOW (fixed), code-simplifier pass after (dead `makeSpinner` fallback removed now both call sites always inject, stale doc comment, one help.mjs line-wrap, two test-assertion messages). MCP surface now 16 tools. 5 remain: `stats`, `history`, `collisions`, `ledger`, `recall_update`/`recall_delete`. Next step: `superpowers:writing-plans` per remaining tool. **`stats`/`history`/`collisions` shipped 2026-08-12** — 9 of the 11, local/not pushed. `stats`/`collisions` needed zero stream-injection work (`run-stats.mjs`/`run-collisions.mjs` already took injectable `print`/`warn`); `history` had no CLI-layer function at all — extracted the inline `bin/ticketlens.mjs` 'history' case into new `run-history.mjs`, same `print`/`warn` shape as its siblings, locked with new tests proving the extraction was byte-identical (golden-diffed `--help` output, real fixture invocation). `collisions` needed the returned `{ok}` for its success signal, not the print/warn-received-content heuristic every other read tool uses — its own failure messages (401/403/no-token/network-error) all write through `print`, not `warn`; same pattern as `callTicketCreate`. Code review caught a CRITICAL, live-reproduced: any `ticket` argument of literally `"--help"`/`"-h"` on `history` matched `printHistoryHelp()`'s own trigger and wrote straight to real `process.stdout` — the JSON-RPC channel itself — corrupting the whole client session. Same root cause traced to **already-published** `fetch`/`compliance`/`pr` (`fetch-ticket.mjs`'s `printFetchHelp()`, same unguarded call), confirmed live and fixed in the same pass with explicit user sign-off to expand scope. `review`/`standup` confirmed unaffected — their user-controlled fields are always embedded inside `--flag=value`, never a bare positional that can equal `--help`. Extracted a shared `callPrintWarnRun` helper for `stats`/`history` (same DRY move `callFetchTicketRun` already made for the fetch family) and split the 250-line `TOOLS` schema array into new `mcp-tool-schemas.mjs` — `mcp-server.mjs` had crossed the 800-line file cap. 3378 tests, 0 failures, code-reviewer 0 CRITICAL after fix / 0 HIGH / 2 LOW (1 fixed — file-size split; 1 accepted — `collisions`' non-fatal insecure-HTTP warning is swallowed under MCP, diagnostic-only, self-hosted edge case). MCP surface now 19 tools. 2 remain: `ledger`, `recall_update`/`recall_delete`. | Large |
| 49c | Feature | **Diagnostic / health-check command (`ticketlens doctor`)** | **Shipped 2026-08-05.** User request: a mechanism to diagnose and check profiles, connections, license, and token validity. Purpose: when TL or the MCP server errors out, evaluate and investigate the failure and produce a report — plus a possible guide toward a fix, or a ready-to-file bug report — instead of leaving the user (or an AI harness calling the MCP tools) to interpret a raw stack trace or a bare non-zero exit. Design spec at `docs/superpowers/specs/2026-08-04-ticketlens-doctor-design.md`: 5 v1 checks (active profile resolution/config validity, adapter connectivity/auth per tracker, license/token freshness tying into the M-12 grace-period logic, plus two more), opt-in `--fix` for 3 of them. MCP server startup/handshake health and MCP registration checks were deliberately deferred out of v1 (memory-only backlog, not on this ROADMAP — see project memory). Next step: `superpowers:writing-plans` to turn the spec into an executable plan. Output shape and tier gating (likely Free, consistent with the existing "free tier tells you the truth when you ask it" philosophy, Phase B.7) are decided in the spec. | Medium–Large |
| 49d | Feature | **Configurable Recall capture strictness** | **Shipped 2026-08-12 (`ticketlens@0.38.29` beta).** User request: a mechanism to set how strict or lenient the jtb skill's Recall-capture nudge (`recall-nudge-stop.mjs` + the "did anything non-obvious come up?" guidance in `SKILL.md`) should be about deciding something is worth saving as a note — not a filter on existing notes, a dial on the judgment call at capture time. Resolved: single scalar (`loose\|balanced\|strict`, default `balanced`), stored per-profile locally in `~/.ticketlens/profiles.json` (not Console-synced — the Stop hook fires on every session end and must stay network-free, unlike the Recall queue settings' rare-path fetch). Enforced by both mechanisms the open question had named: SKILL.md gained three named calibration blocks keyed off a `**Recall capture:**` line injected into the TicketBrief, and the Stop hook's trigger is narrowed for `loose` only — `strict` deliberately never widens the hook's trigger beyond `balanced`'s, since the two ways to do so would each reintroduce a previously-fixed bug (see design spec `docs/superpowers/specs/2026-08-11-recall-capture-strictness-design.md` §5). `note add`'s structural/secret-scan checks were confirmed untouched, as required. Full subagent-driven-development execution (10 tasks + whole-branch review + one fix wave), 3233→3282 tests, 0 regressions. Follow-up filed as backlog #12 (multi-profile users can have the two levers resolve different profiles) — fixed 2026-08-11, `ticketlens@0.38.30`. | Medium |
| 49e | Feature | **Opt-in issue/error reporting** | **Backlog, unscoped (added 2026-08-06).** User request: when TL hits an issue or error, offer to send a diagnostic report to improve the product — gated behind an explicit consent prompt (e.g. "Would you like TicketLens to send reports about issues, bugs, and errors to improve your experience? Y/n"), never silent/automatic. Open questions: where consent is persisted (profile config vs. a new global flag), what a report actually contains (need a redaction pass — likely reuses/extends the `note add` secret-scanner's approach given this session's dogfood findings on that scanner), and whether this hooks into the existing `ticketlens doctor` report path (49c) or is a separate mechanism triggered on uncaught errors. | Medium |
| 49f | Feature | **`ticketlens doctor` progress UX** | **Shipped 2026-08-06 (`ticketlens@0.38.20` beta, current).** User feedback on the shipped 49c: `doctor` gave no visible progress while running and the multi-profile connectivity failure summary dropped its classified hint. Fixed in `0.38.19`: `doctor-command.mjs` now runs checks sequentially with a "○ Checking &lt;label&gt;…" line per check, erased once resolved (`\x1b[A\r\x1b[2K`, same pattern as `login-flow.mjs`), gated on `format === 'plain' && stream.isTTY` — no effect on `--format=json`, piped output, or the MCP path. `connection-tester.mjs`/`doctor-checks.mjs` thread `classifyError`'s `hint` through the full-sweep connectivity summary. Same-day follow-up in `0.38.20`, from live use: the multi-profile summary was still crammed onto one `; `-joined line — changed to `\n`-joined, one profile per row, `renderPlain` updated to indent every line of a multi-line hint (not just the first). Live-verified via `--format=json`, a real pty (`script(1)`), and the global install after each release. 2977/2977 tests pass, code-reviewer: 0 CRITICAL/HIGH both rounds. | Small–Medium |
| 49g | Feature | **Recall: separate local-creation vs. server-push timestamp** | **Backlog, unscoped (added 2026-08-06).** User observation: a Recall note's displayed timestamp is when it was pushed/synced to the Console, not when it was actually captured on the user's machine — the two can differ significantly (e.g. queued notes flushed later, or a backlog of unsynced notes pushed in a batch). Needs a second, distinct "created at" timestamp captured client-side at `note add` time, stored alongside the existing server-side push timestamp, and surfaced in the Console/CLI display. | Small–Medium |
| 49h | Feature | **Recall: screenshot/image attachments from browser tools** | **Backlog, unscoped (added 2026-08-06).** User request: extend Recall notes to store screenshots captured via Chrome DevTools MCP or other browser-automation skills, not just the file/image attachments already shipped for `ticket_create`/`ticket_comment` (2026-08-01, see project memory). Distinct from the already-flagged "Recall note attachments (screenshots/files)" backlog gap — needs scoping on capture flow (which skill/tool triggers the save), storage (same vault/sync path as note bodies?), and size limits. | Medium |
| 49i | Feature | **Pre-fetch and cache a profile/project's valid issue types (CLI + MCP)** | **Backlog, unscoped (added 2026-08-06).** User request: a command or mechanism to fetch a Jira instance/project's actual creatable issue types ahead of time (not just reactively after a `ticket_create` 400), exposed via both CLI and MCP, cached locally — likely alongside or extending `profiles.json`. Extends the existing 2026-07-31 metadata cache and the "populate on type-mismatch failure" follow-up already logged against the `ticket_create` MCP profile bug (see project memory) — this is the proactive counterpart: a ready-lookup rather than only a failure-triggered backfill. Needs scoping: new subcommand (`ticketlens issue-types [--profile=]`?) vs. an MCP tool vs. both; cache location/shape (`profiles.json` vs. a separate cache file, consistent with the existing ticket-metadata-cache pattern); invalidation/refresh policy. | Medium |
| 49j | Feature | **Recall Console: Gmail-style "select all N matching notes" bulk action** | **Backlog, unscoped (added 2026-08-06).** User request: when the header checkbox is used on the Recall admin page, offer a follow-up option to select every matching note across all pages (not just the current page) for bulk delete — matching Gmail's "Select all X conversations that match this search" banner pattern. Confirmed against source: `ticketlens-api/resources/js/Pages/Console/Admin/Recall.vue`'s `toggleSelectAll()` (line 78-79) only ever selects/clears `props.notes.data` — the current page's rows — there is no cross-page "select all" capability today. Needs scoping: a banner/link that appears once all on-page rows are checked (Gmail's exact affordance), a backend bulk-delete-by-filter endpoint (vs. the current presumed by-id-list delete), and confirmation UX for a potentially large destructive action. | Small–Medium |
| 49k | Feature | **`ticketlens doctor` MCP registration + handshake checks** | **Shipped 2026-08-08 (`ticketlens@0.38.21` beta).** Originally scoped out of `doctor` v1 (49c) as two deliberate non-goals — "MCP server startup/handshake health" and "MCP registration check" — pending re-confirmation. Adds `mcp-registration` (reads `.mcp.json` in cwd, reports whether `ticketlens` is registered with the correct command/args; fixable via `ticketlens mcp install`) as an always-on 6th check, and `mcp-handshake` (spawns `ticketlens mcp`, sends a real JSON-RPC `initialize` request, waits up to 5s for a valid response, always kills the child) behind a new opt-in `--mcp` flag — kept opt-in specifically because it's the one with real subprocess/timeout risk, respecting the original deferral's reasoning rather than relitigating it. Also fixed a `lowerFirst()` bug (from 49f) that would have mangled the new "MCP registration"/"MCP server handshake" progress-line labels to "mCP...". 3002/3002 tests, code-reviewer clean (0 CRITICAL/HIGH; 3 MEDIUM + 2 LOW fixed before shipping — DRY duplication between `doctor-checks.mjs` and `mcp-install.mjs` on the "desired entry" shape, a test that didn't test its own title, a defensive `stdin` error handler, Windows portability, timeout margin). Live-verified against the real global install: `--mcp` handshake, default registration check, and `--fix` all confirmed working end-to-end, not just mocked. | Small–Medium |
| 49l | Feature | **`ticketlens doctor` progressive checklist loader (revises 49f)** | **Shipped 2026-08-08 (`ticketlens@0.38.22` beta).** User feedback on 49f's static flash-and-erase loader: with 6-7 checks each resolving in milliseconds, only the one slow check (tracker connectivity) was ever visible — everything else looked frozen, then the whole report dumped at once. Replaced with an animated, incrementally-revealed checklist: each check gets an animated spinner (reused `banner.mjs`'s `SPINNER_FRAMES`/`SPINNER_INTERVAL`, now exported), morphs in place into its own ✔/✖ row once resolved, and that row is appended (never erased again) — the report builds top-down as checks finish instead of dumping at the end. Deliberate "Option B" design for `--fix`: a repaired check's already-shown row keeps its raw pre-fix state; only the trailing `Fixed:` line signals the repair (no precedent anywhere in this codebase for rewriting an arbitrary earlier terminal row — would require buffering and redrawing every row printed after it). `--help` gained a one-line clarification of this. Code review caught 1 HIGH (no SIGINT handler — Ctrl+C mid-check could leave a user's real cursor permanently hidden; fixed to match the existing `init-wizard.mjs` precedent) and 1 MEDIUM (the `--fix` row-vs-summary UX gap, addressed via the help-text line) before shipping. 3008/3008 tests including a real ~250ms-delay spinner-cycling test and a synchronous mid-run `out.text` inspection proving genuine incremental reveal (not just end-state equivalence). Live-verified via a real pty (`script(1)`) — actual terminal bytes confirmed cursor hide/show, spinner frame, erase, and top-down row accumulation all render correctly. | Small–Medium |
