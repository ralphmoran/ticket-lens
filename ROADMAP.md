...
| ~~20~~ | ~~Feature~~ | ~~**Live Console Events (SSE)**~~ | Done. SSE endpoint `GET /console/events` (Pro+ tier, session-cookie auth, Redis Streams `XREAD BLOCK 25s`). `SseEventService::publish()` fire-and-forget called in `RulesController` (`rule.changed`) and `PushController` (`triage.pushed`). Frontend: Pinia `useEventsStore`, `useServerEvents()` composable (EventSource lifecycle, tier-gated, auto-reconnect on drop), wired in `ConsoleLayout`. 9 new tests across `SseControllerTest`, `RulesControllerTest`, `PushControllerTest`. | ~~Medium~~ |
