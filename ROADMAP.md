...
| 49f | Feature | **`ticketlens doctor` progress UX** | **Shipped 2026-08-06 (`ticketlens@0.38.19` beta).** User feedback on the shipped 49c: `doctor` gave no visible progress while running and the multi-profile connectivity failure summary dropped its classified hint. Fixed: `doctor-command.mjs` now runs checks sequentially with a "○ Checking &lt;label&gt;…" line per check, erased once resolved (`\x1b[A\r\x1b[2K`, same pattern as `login-flow.mjs`), gated on `format === 'plain' && stream.isTTY` — no effect on `--format=json`, piped output, or the MCP path. `connection-tester.mjs`/`doctor-checks.mjs` thread `classifyError`'s `hint` through the full-sweep connectivity summary. Same-day follow-up from live use: the multi-profile summary was still crammed onto one `; `-joined line — changed to `\n`-joined, one profile per row, `renderPlain` updated to indent every line of a multi-line hint (not just the first). Live-verified via `--format=json`, a real pty (`script(1)`), and the global install. 2977/2977 tests pass, code-reviewer: 0 CRITICAL/HIGH both rounds. | Small–Medium |
