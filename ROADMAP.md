...
| 49c | Feature | **Diagnostic / health-check command (`ticketlens doctor`)** | **Backlog, unscoped (added 2026-08-04).** User request: a mechanism to diagnose and check profiles, connections, license, and token validity. Purpose: when TL or the MCP server errors out, evaluate and investigate the failure and produce a report — plus a possible guide toward a fix, or a ready-to-file bug report — instead of leaving the user (or an AI harness calling the MCP tools) to interpret a raw stack trace or a bare non-zero exit. Likely checks: active profile resolution and config validity, adapter connectivity/auth per tracker (Jira/GitHub/Linear), license/token freshness (ties into the M-12 grace-period logic), and MCP server startup/handshake health. Open design questions, none resolved yet: exact check list and how deep each goes (e.g. does it make a live network call per adapter, or only validate local config); where the diagnostic logic lives so both a CLI `ticketlens doctor` and an MCP tool can share one implementation (same precedent as 46b's ticket write-back and 49b's shared-tool-surface pattern); output shape — human-readable report vs. structured JSON an AI harness can act on; whether it only diagnoses + guides or also offers to auto-fix (e.g. re-run `activate`, clear a stale cache); and tier gating — likely Free, consistent with the existing "free tier tells you the truth when you ask it" philosophy (Phase B.7), since gating basic troubleshooting behind Pro would increase support burden rather than reduce it. | Medium–Large |
