...
export function printMcpHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `  ${s.cyan('ticket_transition')}, ${s.cyan('ticket_assign')}, ${s.cyan('ticket_duplicates')}, ${s.cyan('ticket_link')}, ${s.cyan('ticket_update')} — for any`,
    `  MCP-compatible AI harness, not just Claude Code. Thin adapter over the same`,
    `  code as ${s.cyan('note add')}/${s.cyan('recall')}/${s.cyan('comment')}/${s.cyan('transition')}/${s.cyan('assign')}/${s.cyan('duplicates')}/${s.cyan('link')}/${s.cyan('update')} above:`,
