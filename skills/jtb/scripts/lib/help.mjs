...
export function printHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `    ${s.cyan('--check')}            Append VCS diff + review instructions for Claude Code`,
    `    ${s.cyan('--summarize')}        Generate AI summary ${s.dim('(BYOK or --cloud) [Pro]')}`,
    `    ${s.cyan('--cloud')}            Route summary through TicketLens API ${s.dim('[Pro]')}`,
    '',
    ...
    `    ${s.cyan('--export')}=${s.dim('FORMAT')}   Export results to file  ${s.dim('(csv|json) [Team]')}`,
    `    ${s.cyan('--digest')}           POST scored results to digest endpoint  ${s.dim('[Pro]')}`,
