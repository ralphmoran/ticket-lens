...
export function printProfilesHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `        for accounts that belong to more than one team. Omit the team name`,
    `        to pick one interactively from your synced teams. Stored by id,`,
    `        refreshed by ${s.cyan('ticketlens login')}/${s.cyan('ticketlens sync')}.`,
    '',
    `  ${s.bold('OPTIONS')}`,
    '',
...
export function printNoteHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    '',
    `  ${s.bold('ticketlens note patch')} ${s.dim('--id="..." [--ticket=KEY] [--expect-mtime=MS]')}  ${s.dim('[Pro]')}`,
    '',
    ...
    `  ${s.bold('OPTIONS')}`,
    '',
    `    ${s.brand('--id')}=${s.dim('ID')}             Note id to patch ${s.dim('(required)')}`,
    `    ${s.brand('--ticket')}=${s.dim('KEY')}         Ticket this note is about ${s.dim('(optional)')}`,
    `    ${s.brand('--expect-mtime')}=${s.dim('MS')}    File mtime (ms) the caller last observed ${s.dim('(optional)')} — a no-op,`,
    `                          not an overwrite, if the file changed since then`,
    '',
...
export function printMcpHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `  ${s.cyan('compliance')}'s \`consensus: true\` (Pro) runs a multi-agent review server-side against`,
    `  a "consensus" role configured at Console > Admin > AI Roles, and always implies`,
...
export function printDuplicatesHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `  On Jira, any ticket already linked as a "Duplicate" is always listed first —`,
...
export function printTriageHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `    ${s.brand('--static')}           Static table output ${s.dim('(skip interactive mode)')}`,
    `    ${s.brand('--plain')}            Plain markdown output`,
    `    ${s.brand('--styled')}           Force ANSI-styled output ${s.dim('(even when piped)')}`,
    `    ${s.brand('-h')}, ${s.brand('--help')}         Show this help`,
    '',
    `  ${s.bold('EXAMPLES')}`,
    '',
...
export function printComplianceHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `    ${s.dim('               ')}    — requires ${s.cyan('ticketlens login')} and a "consensus" role configured`,
    `    ${s.dim('               ')}    at Console > Admin > AI Roles (2+ providers from your team's`,
    `    ${s.dim('               ')}    shared pool, Console > Admin > AI Provider Pool). Runs server-side`,
    `    ${s.dim('               ')}    — the diff and requirements are sent to TicketLens's backend,`,
    `    ${s.dim('               ')}    scanned for secrets first, then to each provider. Reconciles`,
    `    ${s.dim('               ')}    disagreements with a refinement round, then a majority vote.`,
    `    ${s.dim('               ')}    Prompts for confirmation before spending your team's API credits.`,
