...
export function printMcpHelp({ stream = process.stdout } = {}) {
  ...
  const lines = [
    ...
    `  native tools — ${s.cyan('fetch')}, ${s.cyan('doctor')}, ${s.cyan('triage')}, ${s.cyan('recall_add')}, ${s.cyan('recall_search')}, ${s.cyan('ticket_comment')},`,
    ...
    `  code as ${s.cyan('TICKET-KEY')}/${s.cyan('doctor')}/${s.cyan('triage')}/${s.cyan('note add')}/${s.cyan('recall')}/${s.cyan('comment')}/${s.cyan('transition')}/${s.cyan('assign')}/${s.cyan('duplicates')}/${s.cyan('link')}/${s.cyan('update')}/${s.cyan('create')}`,
    `  above. ${s.cyan('fetch')} and ${s.cyan('doctor')} are Free; ${s.cyan('triage')} is Free with some Pro/Team-gated`,
    `  options (see ${s.cyan('ticketlens triage --help')}); every other tool needs Pro. ${s.cyan('ticket_transition')} is`,
